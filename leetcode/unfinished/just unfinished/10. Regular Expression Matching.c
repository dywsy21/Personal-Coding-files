...
void asterisk(int* sindex, int* pindex,char* s, char* p ,int* det){  //p[pindex]=='*'
    ...
    if((*pindex)-1>=0 && p[(*pindex)-1]=='.'){
        ...
        while(s[(*sindex)]==p[(*pindex)-1]&&(*sindex)<strlen(s)-1)   (*sindex)++;
        (*pindex)++;
        return;
    }
}
...
bool isMatch(char *s, char *p) {
    ...
    while (pindex<strlen(p) && sindex<strlen(s)) {
        if (pindex<strlen(p) && sindex<strlen(s) && p[pindex] == '.') {
            pindex++;
            sindex++;
        }
...
DEBUG
            ...
            if (det==2)
                return false;
            if((sindex==strlen(s)||sindex==strlen(s)-1) && pindex==strlen(p))  return true;
        }

        if (pindex<strlen(p) && sindex<strlen(s) && !(p[pindex] == '*' || p[pindex] == '.')) {
...
DEBUG
    if(sindex==strlen(s) && pindex==strlen(p)-2 && p[pindex]!='*' && p[pindex+1]=='*')
    return true;

    if(sindex==strlen(s) && pindex==strlen(p)-2 && p[pindex]=='*' && p[pindex+1]==s[sindex-1])
    return true;


    if((pindex==strlen(p)||pindex==strlen(p)-1)&&sindex==strlen(s))   return true;
    else return false;
}
